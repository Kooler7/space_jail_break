...
func start_dialogue() -> void:
	if _current_dialogue_tree != null:
		...
		Globals.current_object.set_mouse_detector_pickability(false)
		_current_dialogue_node = _current_dialogue_tree.get_node_safe(START_DIALOGUE_NODE)
...
func change_dialogue_node() -> void:
	...
	if _current_dialogue_node.next_node != "END":
		_current_dialogue_node = _current_dialogue_tree.get_node_safe(_current_dialogue_node.next_node)
		check_node_type()
...
func execute_randomizer() -> void:
	...
	_current_dialogue_node = _current_dialogue_tree.get_node_safe(picked_next_node)
	check_node_type()
	...
	match _current_dialogue_node.current_action_type:
		...
		_current_dialogue_node.ActionTypes.SET_LEVEL_FLAG:
			for flag in _current_dialogue_node.flags:
				_set_level_global_flag(GameState.level_flags, flag.key, flag.value)
		...
		_current_dialogue_node.ActionTypes.SET_GLOBAL_FLAG:
			for flag in _current_dialogue_node.flags:
				_set_level_global_flag(GameState.global_flags, flag.key, flag.value)
		...
		_current_dialogue_node.ActionTypes.CHANGE_DIALOGUE_TREE:
			_current_dialogue_tree = Globals.current_object.check_available_dialogue_tree()
			if _current_dialogue_tree != null:
				_current_dialogue_node = _current_dialogue_tree.get_node_safe(START_DIALOGUE_NODE)
			elif _current_dialogue_tree == null:
				return
...
func _set_level_global_flag(new_storage: Dictionary, new_flag: String, new_value: bool) -> void:
